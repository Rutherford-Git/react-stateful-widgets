...
const [feat, setfeat]= useState(null);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: feat ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          prog.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setfeat(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          feat ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳`: 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
